<div class="w-full h-full px-2 ">
    <div class= "flex justify-between h-10 mb-3">
        <button wire:click="$dispatch('open-new-pei')" class='focus:outline-none px-4 py-1 dark:text-gray-900 dark:bg-gray-100 text-white rounded-md hover:outline-none bg-teal-600 hover:bg-teal-800' >
            + {{ __('ent.add-versement') }}
        </button>
    </div>
    ...
    @forelse ($paiements as $paiement)
        <tr @class(['h-4','w-full','text-center','bg-red-500/20' => $paiement->list, 'dark:bg-red-900/30' => $paiement->list])  >
            ...
            <td class="flex space-x-2 py-2 px-2 h-full items-center">
                <div class="mx-4">
                    <button   wire:click="$dispatch('dett-edit-pei',{ id: {{ $paiement->id }} })">
                ...
                <div>
                    @can('admin')
                        <button  wire:click="$dispatch('dtls-pei',{ id: {{ $paiement->id }} })" class=" stroke-red-600">
