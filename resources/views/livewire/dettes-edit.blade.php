                ...
                <div class="grid lg:grid-cols-2 gap-x-6 gap-y-3 px-12  justify-items-center sm:grid-cols-1 ">
                  <div class="flex flex-col space-y-1 w-full">
                      <div class="flex justify-between">
                        <label for="eid"  class="labels">{{ __('ent.montnat') }}:</label>
                        @error('montant') <span class="danger">{{ $message }}</span> @enderror  
                      </div>
                      <input dir="ltr" wire:model='montant' x-model='montant' x-mask:dynamic="$money($input, '.', ' ')"  class="inputs w-full @error('montant') reds @enderror" type="text"   required  />
                  </div>

                  <div  class="flex flex-col space-y-1  w-full">
                    <div class="flex justify-between">
                      <label   class="labels">{{ __('ent.date') }}:</label>
                    ...
                    <div class="flex flex-col space-y-1 col-span-2 w-full">
                        <div class="flex justify-between">
                          <label   class="labels">{{ __('ent.type') }}:</label>
                          @error('type') <span class="danger">{{ $message }}</span> @enderror  
                        </div>
                        <select x-model='select' wire:model.defer="type" class="inputs w-full @error('type') reds @enderror">
                          <option value="">-----</option>
                                  <option value="0"> {{ __('ent.free') }} </option>
                                  <option value="1"> {{ __('ent.pret') }} </option>
                        </select> 
                    </div>

                    <div x-show='parseInt(select)' class="flex flex-col space-y-1 w-full">
                      <div class="flex justify-between">
                        <label   class="labels">{{ __('ent.date-debut') }}:</label>
                        @error('datep') <span class="danger">{{ $message }}</span> @enderror  
                      </div>
                      <input wire:model.defer='datep' class="inputs w-full @error('datep') reds @enderror" type="date" required  />
                    </div>

                    <div x-show='parseInt(select)' class="flex flex-col space-y-1 w-full">
                      <div class="flex justify-between">
                        <label   class="labels">{{ __('ent.echs') }}:</label>
                        @error('echances') <span class="danger">{{ $message }}</span> @enderror  
                      </div>
                      <input  wire:model.defer='echances' class="w-full inputs @error('echances') reds @enderror" type="text" required  />
                  </div>


                  <div x-show='parseInt(select)' class="flex flex-col space-y-1 w-full">
                      <div class="flex justify-between">
                        <label   class="labels">{{ __('ent.intret') }} (%):</label>
                        @error('intret') <span class="danger">{{ $message }}</span> @enderror  
                      </div>
                      <input  x-model='intret' wire:model.defer='intret' class="inputs  w-full" type="number" required  />
                  </div>

                  <div x-show='parseInt(select)' class="flex flex-col space-y-1 w-full">
                      <div class="flex justify-between">
                        <label   class="labels ">{{ __('ent.montnat') }} + {{ __('ent.intret') }}</label>
                        @error('intret') <span class="danger">{{ $message }}</span> @enderror  
                      </div>
                      <input  x-model='(parseInt(montant.split(" ").join("")) + parseInt(montant.split(" ").join(""))*(parseInt(intret)/100))' x-mask:dynamic="$money($input, '.', ' ')" disabled class="inputs w-full" type="text" required  />
                  </div>
              </div>
