<div>
  <x-dialog-modal wire:model='visible' >
      <x-slot name='title'>
       ...
       <x-slot name='content'>
          <div x-data="{
            editpaiements: @entangle('paiements'),


            addPaiementedit() {
                // Add a new empty paiement to the Alpine.js array
                this.editpaiements.push({ etudiant_id: '', month: '', montant: null });
            },

            removePaiementedit(index) {
                // Remove the element from the Alpine.js array
                this.editpaiements.splice(index, 1);
            },

            getTotal() {
              const total = this.editpaiements.reduce((total, paiement) => {
           ...
           class="flex flex-col space-y-8 ">
              ...
              <div class="grid lg:grid-cols-2 gap-x-6 gap-y-3 px-12  justify-items-center">
                    ...
                    <div class=" w-full flex flex-col space-y-2">
                      ...
                      <input dir="ltr" :value="getTotal()"  disabled   class="inputs w-full  @error('paiements.*') reds @enderror " type="text"  /> 
                    </div> 
                    ...
                    <div class="flex flex-col space-y-1 col-span-2 w-full">
                      ...
                      <div class="flex flex-col  gap-1  w-full">
                              ...
                              <div class="flex gap-1 w-full">
                                  <select wire:model="paiements.{{$index}}.etudiant_id" name="paiements.{{$index}}.etudiant_id" class="inputs w-56 @error('type') reds @enderror"  >
                                      ...
                                      @foreach ($etuds as $item)
                                          <option  class="text-sm" value="{{ $item->id }}">
                                              @if (app()->getLocale() == 'ar')
                                                  {{$item->nom}}
                                              @else
                                                  {{$item->nomfr}}
                                  ...
                                  <input wire:change="totals" wire:model="paiements.{{$index}}.montant" @change="calculateTotal()"  class="inputs w-24 @error('rdate') reds @enderror" type="text"   />    
                                  @if ($loop->first)
                              ...
                              <div  >
                                <template class="flex flex-col  gap-1  w-full" x-for="(paiement, index) in editpaiements" :key="index">

                                  <div class="flex gap-1 w-full my-1">
                                    <select x-model="paiement.etudiant_id"  class="inputs w-56  "  >
                                    ...
                                    <button x-show="index == 0" class="text-green-600  dark:text-green-400" @click="addPaiementedit">
                                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v6m3-3H9m12 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                                      </svg>                                                                      
                                    </button>
                                    
                                    <button x-show="index != 0" class="text-red-600  dark:text-red-400" @click="removePaiementedit(index)">
                                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                          <path stroke-linecap="round" stroke-linejoin="round" d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                                      </svg>                                  
                                  </button>
