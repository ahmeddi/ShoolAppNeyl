         ...
         <x-slot name='content'>
            <div class="flex flex-col space-y-4 ">
                ...
                <div class="grid lg:grid-cols-2 gap-x-6 gap-y-3 px-12  justify-items-center sm:grid-cols-1 ">
                    ...
                    <div class="flex flex-col space-y-1 col-span-2 w-full ">
                        <div class="flex justify-between">
                          <label for="eid"  class="labels">{{ __('prof.prof-mat-class') }}:</label>
                    ...
                    <div class="flex flex-col space-y-1 w-full">
                        <div class="flex justify-between">
                            <label for="eid"  class="labels">{{ __('prof.prof-mat-type') }}:</label>
                            @error('type') <span class="danger ">{{ $message }}</span> @enderror  
                          </div>
                          <select  wire:model.defer="type"   class="inputs w-full @error('type') reds @enderror">
                              <option value="">-----</option>
                              <option value="1"> {{ __('prof.prof-class-rel') }} </option>
                              <option value="2"> {{ __('prof.prof-class-her') }} </option>
                              <option value="3"> {{ __('prof.prof-class-fix') }} </option>
                          </select>        
                    </div>

                    <div class="flex flex-col space-y-1 ">
                        <div class="flex justify-between">
                          <label for="eid"  class="labels">{{ __('prof.prof-mat-val') }}:</label>
                          @error('prix') <span class="danger ">{{ $message }}</span> @enderror  
                        </div>
                        <input wire:model.defer='prix' class="inputs  col-span-2 @error('prix') reds @enderror" type="text"   required  />       
                    </div>
