        ...
        <x-slot name='title'>
         ...
         <x-slot name='content'>
            <div  class="flex flex-col space-y-4 ">
                ...
                {{--  top rows --}}
                <div class="grid lg:grid-cols-2 gap-x-6 gap-y-4 px-12  justify-items-center sm:grid-cols-1 ">
                    <div class="flex flex-col space-y-1 w-full">
                        <div class="flex justify-between">
                          <label  class="labels">{{ __('users.name') }}:</label>
                    ...
                    <div class="flex flex-col space-y-1 w-full">
                        <div class="flex justify-between">
                          <label   class="labels">{{ __('users.role') }}:</label>
                          @error('user') <span class="danger">{{ $message }}</span> @enderror  
                        </div>
                        <select  wire:model.defer='user'  class="inputs w-full @error('user') reds @enderror" name="user"  required >
                            <option class="text-sm" value="0" >-----</option>
                            <option  class="text-sm" value="admin">{{ __('users.admin') }}</option>
                            <option  class="text-sm" value="comps">{{ __('users.comps') }}</option>
                            <option  class="text-sm" value="sur">{{ __('users.sur') }}</option>
                            <option  class="text-sm" value="dir">{{ __('users.dir') }}</option>
                        </select>
                    </div>

                    <div class="flex flex-col space-y-1 col-span-2 w-full">
                        <div class="flex justify-between">
                          <label for="eid"  class="labels">{{ __('users.password') }}:</label>
                          @error('password') <span class="danger">{{ $message }}</span> @enderror  
                        </div>
                      <input wire:model.defer='password' class="inputs w-full @error('password') reds @enderror" type="text"   required  />    
                    </div>
