  ...
  <x-dialog-modal wire:model.defer='visible' >
    <x-slot name='title'>
      <div class='relative w-full px-12 text-lg font-bold text-green-900 dark:text-gray-50 '>
       ...
       <x-slot name='content'>
          <div x-data="{telephone: @entangle('telephone'), telephone2:@entangle('telephone2') ,whatsapp: @entangle('whatsapp')}" class="flex flex-col space-y-4 ">
              ...
              <div class="grid lg:grid-cols-2 gap-x-6 gap-y-3 px-12  justify-items-center sm:grid-cols-1 ">
                  <div class="flex flex-col space-y-1 col-span-2 w-full">
                      <div class="flex justify-between">
                        <label for="eid"  class="labels"> {{ __('ent.nom') }}:</label>
                        @error('nom') <span class="danger">{{ $message }}</span> @enderror  
                      </div>
                    <input wire:model.defer='nom' class="inputs w-full @error('nom') reds @enderror" type="text"   required  />    
                  </div>

                  <div  class="flex flex-col space-y-1">
                    <div class="flex justify-between">
                      <label for="eid"  class="labels">{{ __('ent.tel') }}:</label>
                ...
                <div class="flex flex-col space-y-1">
                    <div class="flex justify-between">
                      <label   class="labels">{{ __('ent.wh') }}:</label>
                      @error('whatsapp') <span class="danger">{{ $message }}</span> @enderror  
                    </div>
                    <input  dir="ltr"  x-model="whatsapp" x-mask="99 99 99 99" wire:model.defer='whatsapp' class="inputs @error('whatsapp') reds @enderror" type="text" required  />
                </div>

                <div class="flex flex-col space-y-1">
                  <div class="flex justify-between">
                    <label   class="labels">{{ __('ent.tel2') }}:</label>
                    @error('telephone2') <span class="danger">{{ $message }}</span> @enderror  
                  </div>
                  <input  dir="ltr"  x-model="telephone2" x-mask="99 99 99 99" wire:model.defer='telephone2' class="inputs @error('telephone2') reds @enderror" type="text" required  />
                </div>

                  <div class="flex flex-col space-y-1 ">
                      <div class="flex justify-between w-full">
                        <label   class="labels"> {{ __('ent.email') }}:</label>
                        @error('email') <span class="danger">{{ $message }}</span> @enderror  
                      </div>
                      <input wire:model.defer='email' class="inputs @error('email') reds @enderror" type="email" required  />
                  </div>
              </div>
          </div>  
