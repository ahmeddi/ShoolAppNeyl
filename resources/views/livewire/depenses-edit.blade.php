<div x-data="{ montant: @entangle('montant'),}">
      ...
      <x-slot name='title'>
       ...
       <x-slot name='content'>
          <div class="flex flex-col space-y-8 ">
              ...
              {{-- 4 top rows --}}
              <div  x-data="{ent: @entangle('ent') }"class=" grid lg:grid-cols-2 gap-x-6 gap-y-3 px-12  justify-items-center">
                <div class="col-span-2 w-full flex flex-col space-y-2">
                  <div class="flex justify-between ">
                      <label   class="labels">{{ __('compt.fix') }}  :</label>
                      @error('ent') <span class="danger">{{ $message }}</span> @enderror  
                  </div>
                  <div class="flex">
                    <select x-model="ent"  wire:model.defer="ent"   class="inputs w-full @error('ent') reds @enderror">
                      <option value="0" >-----</option>
                          @forelse ($ents as $value)
                ...
                <div x-show="!parseInt(ent)" class="col-span-2 w-full flex flex-col space-y-2">
                  <div class="flex justify-between ">
                      <label   class="labels">{{ __('compt.lib') }}  :</label>
                      @error('titre') <span class="danger">{{ $message }}</span> @enderror  
                  </div>
                    <input  wire:model.defer='titre' class="inputs w-full @error('titre') reds @enderror" type="text"   required  />  
                </div>

                <div class="flex w-full flex-col space-y-2">
                      <div class="flex justify-between w-full">
                        <label   class="labels">{{ __('compt.date') }}  :</label>
                        @error('date') <span class="text-red-500 text-sm font-light">{{ $message }}</span> @enderror  
                      </div>
                    <input wire:model='date' id="date" class="inputs" type="date"  name="date"  required autofocus />    
                </div>
            
                <div class="flex w-full flex-col space-y-2">
                    <div class="flex justify-between w-full">
                        <label for="eid"  class="labels">{{ __('compt.mont') }}   :</label>
                        @error('montant') <span class="text-red-500 text-sm font-light">{{ $message }}</span> @enderror  
                      </div>
                ...
                <div class="col-span-2 w-full">
                    <div class="flex flex-col space-y-2">
                            <div class="flex justify-between">
                                <label   class="labels">{{ __('compt.note') }}  :</label>
                              </div>
                              <textarea wire:model='detail' class="textearea h-32 w-full @error('detail') reds @enderror" type="text" required  ></textarea>
                    </div>                    
                </div>
