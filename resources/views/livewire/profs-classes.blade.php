<div class="w-full h-full px-2  ">
    <div class= "flex justify-between my-4">
        @can('dir')
            <button wire:click="$dispatch('pattl',{ids:{{ $ids }} })" class='focus:outline-none px-4 py-2 dark:text-gray-900 dark:bg-gray-100 text-white rounded-md hover:outline-none bg-teal-600 hover:bg-teal-800' >
    ...
    <table class="w-full divide-y divide-gray-200 dark:divide-gray-600 rounded-t-xl overflow-hidden">
        ...
        <tr class=" rtl:text-right ltr:text-left">
            ...
            <th scope="col" class="px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                {{ __('prof.prof-mat-class') }}
            </th>
            <th scope="col" class="px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
               {{ __('prof.prof-mat-type') }}
            </th>
            <th scope="col" class="px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                {{ __('prof.prof-mat-val') }}
             </th>
            <th>
                ...
                @forelse ($records as $record)
                    <tr class="h-5 ">
                                ...
                                <div class="ml-2 flex space-x-2 ">
                                    <div class="flex flex-col">
                                        <div class="text-sm font-semibold text-gray-900 dark:text-gray-200">
                                            ...
                                            <a class="">
                                                 @if($record->type == 1)
                                                    {{ __('prof.prof-class-rel') }}
                                                 @elseif($record->type == 2)
                                                    {{ __('prof.prof-class-her') }}
                                                 @elseif($record->type == 3)
                                                    {{ __('prof.prof-class-fix') }}
                                                 @endif
                                            </a>
                                        </div>
                                    </div>
                        ...
                        <td class="flex space-x-2 py-2 h-full items-center">
                            @can('dir')
                                <div class="mx-4">
                                    <button wire:click="$dispatch('profcedit',{id:{{ $record->id }} })" >
                                ...
                                <div>
                                    <button   wire:click="$dispatch('profcdel',{id:{{ $record->id }} })" class=" stroke-red-600">
